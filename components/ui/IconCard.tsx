...
export function IconCard({ icon: Icon, title, desc, href, tone = 'teal', meta }: { icon: LucideIcon; title: string; desc: string; href?: string; tone?: Tone; meta?: string }) {
  const body = (
    <div className="p-6">
      <div className="flex items-start justify-between gap-4">
        <div className={`icon-tile ${toneClass[tone]}`}><Icon className="h-6 w-6" aria-hidden="true" /></div>
        {href && <span className="flex items-center gap-1 text-sm font-medium text-primary">{meta ?? 'Learn more'}<ArrowUpRight className="h-4 w-4 rtl:-scale-x-100" aria-hidden="true" /></span>}
      </div>
