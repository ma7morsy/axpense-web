...
export function ProductScreenshot({ priority = false, compact = false }: { priority?: boolean; compact?: boolean }) {
  return (
    <figure className={`overflow-hidden rounded-xl border border-border bg-card shadow-elevated ${compact ? '' : 'p-1.5 sm:p-2'}`}>
      ...
      <Image
        ...
        alt="Axpense fleet management dashboard showing vehicles, maintenance, fuel, expenses and fleet costs"
        width={1536}
        height={1024}
        priority={priority}
      ...
      />
      <figcaption className="sr-only">Axpense fleet management dashboard</figcaption>
    </figure>
  );
}
